...
async function main(): Promise<void> {
  ...
  host.emit(EVT.hostSelectMinigame, { minigameId: 'kart3d' });
  await sleep(200);
  ...
  if (pick.minigameId !== 'kart3d') throw new Error('kart3d non selezionato');
  if (pick.players.length !== 5) throw new Error('attesi 5 giocatori');

  console.log('✅ KART 3D TEST OK (5 giocatori)');
  process.exit(0);
}
