...
| 组件 | 一句话 | 在哪个模块动手 |
|---|---|---|
| **Build** | 你上传的服务器二进制 + 安装脚本 | 模块 3 |
| **Fleet（托管 EC2）** | AWS 托管的实例，运行你的服务器进程 | 模块 3 |
| **Fleet（Anywhere）** | *你自己的*硬件注册为 fleet 算力 | 附录 A（可选） |
| **Game Session** | 某个服务器进程上正在运行的一局比赛 | 模块 3–5 |
| **Game Session Queue** | 决定每一局新会话放到哪个 fleet/位置 | 模块 3 |
| **FlexMatch** | 按规则把玩家分组，然后请 Queue 开局 | 模块 4 |
