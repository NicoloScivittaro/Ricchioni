...
```
 QUEUED → SEARCHING → PLACING → COMPLETED
              │                 └─ 连接信息：
              │                     IP/DNS + 端口 + PlayerSessionId
              └─ REQUIRES_ACCEPTANCE （仅在启用接受确认时出现；
                                       本 workshop 不需要接受确认）
 失败路径：TIMED_OUT · CANCELLED · FAILED
```

`PotentialMatchCreated` 是 FlexMatch 的**事件**（match 被提议时发出），并不是
票据状态。由于本 workshop 不需要玩家接受确认，票据直接从 `SEARCHING` 进入 `PLACING`。
