...
*只读——打开文件对照阅读，不需要修改。fleet 还在激活中，它此刻安装的就是这份代码。*

在编辑器中打开 **`server/gamelift/manager.go`**。这一个文件包含了我们 Go
游戏服务器的全部 Amazon GameLift Servers 集成。对照模块 1 的生命周期图，逐一看真实代码。
...
在你刚部署的这个 fleet 上，`m.Anywhere` 是 `nil`，`params` 保持**零值**——
GameLift 把 fleet、compute 和鉴权信息放在实例环境变量里，SDK 自己读取。
两种托管模式的区别就在这里：只有当算力不是 AWS 托管实例时，才需要填满这个
结构体（可选的附录 A）。这一行之后的所有代码，两种模式完全一致。

{{% notice info %}}
注意方向：服务器进程通过 WebSocket **主动连出**到 GameLift。GameLift 从不
*连入*来管理它——同样的道理，NAT 后面的笔记本也能当 Anywhere compute。
{{% /notice %}}
