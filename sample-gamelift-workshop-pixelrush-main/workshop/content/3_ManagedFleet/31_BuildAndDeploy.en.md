...
come in Module 4). Deploying only the Amazon GameLift Servers stack would leave the backend in
its previous mode.
...
The `-c stage=ec2` flag extends the backend you deployed in Module 2:
...
| **Queue** | `PixelRushQueue` — the placement target for sessions (direct in this module, via FlexMatch in Module 4) |
...
health checks). Don't wait idle — the next three pages are written for exactly
this window: how the fleet is configured, the Server SDK code the fleet is
installing, and how the queue picks a location. Come back when the deploy
command returns.
{{% /notice %}}
