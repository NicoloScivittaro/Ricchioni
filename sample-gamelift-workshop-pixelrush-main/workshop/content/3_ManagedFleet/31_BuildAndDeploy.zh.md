...
**直接放置**到 fleet 上，**没有任何匹配规则**（FlexMatch 规则在模块 4 才引入）。只部署
Amazon GameLift Servers stack 会让后端停留在上一个模式。
