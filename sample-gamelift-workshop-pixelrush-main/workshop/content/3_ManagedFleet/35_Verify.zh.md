...
预期：`UPDATE_COMPLETE`（首次部署则是 `CREATE_COMPLETE`）。

{{% notice warning %}}
本模块剩下的**每一条** `cdk` 命令都要带上 `-c stage=ec2`。`cdk deploy` 会连同目标
stack 依赖的其他 stack 一起部署，而前端 stack 依赖后端 stack——一旦漏掉这个 flag，
CDK 就会把后端重新合成成 FlexMatch 模式，悄悄把你刚设好的直接放置覆盖掉。而此时
EC2 的 FlexMatch 配置还不存在（那是模块 4 才创建的），对战会失败且看不出原因。
到了模块 4，这个 flag 换成 `-c stage=ec2-match`。
{{% /notice %}}
...
前端本身不用改——它仍然调用你同一个 API，只是现在比赛被路由到你的托管 fleet 上。
...
4. 两个标签会进入托管 fleet 上的**同一局**——倒计时，然后开赛。没有任何证书
   告警：fleet 有 GameLift 签发的 TLS 证书，客户端的 `wss://` 连接自动受信任。
