...
Confirm the stack finished, the same way you did for the backend in Module 2:
...
Expected: `UPDATE_COMPLETE` (or `CREATE_COMPLETE` the first time).

{{% notice warning %}}
Keep `-c stage=ec2` on **every** `cdk` command for the rest of this module.
`cdk deploy` also deploys the stacks your target depends on, and the frontend
stack depends on the backend — drop the flag and CDK re-synthesizes the backend
in FlexMatch mode, silently undoing the direct placement you just set up. There
are no EC2 FlexMatch configurations yet (those arrive in Module 4), so races
would fail with no obvious cause. In Module 4 the flag becomes
`-c stage=ec2-match`.
{{% /notice %}}
...
The frontend itself needs no change — it still calls your same API, which now
routes races onto your managed fleet.
...
4. Both tabs land in the **same race** on your managed fleet — countdown, then
   go. No certificate warnings: the fleet has a GameLift-issued TLS cert, so the
   client's `wss://` connection is trusted automatically.
...
front of this same fleet — is Module 4.
{{% /notice %}}
