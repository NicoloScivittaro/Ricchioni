...
fleet 跨**多个区域**（附录 B 会加入一个远端 location），queue 就要*做选择*——这时
...
`PriorityConfiguration` 会对四种放置策略排序，Amazon GameLift Servers 按你列出的先后依次应用，
后一条用来打破前一条的平局：
...
一个地方可去。附录 B 会加入一个远端 location——做完之后，这条 queue 策略就会自动把
