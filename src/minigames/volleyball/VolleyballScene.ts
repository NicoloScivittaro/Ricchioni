...
export class VolleyballScene extends Phaser.Scene {
  ...
  create(data: { ctx: MinigameContext }): void {
    ...
    this.pauseMenu = new VolleyballPauseMenu(this, () => this.scene.restart({ ctx: data.ctx }));
  ...
  private async boot(ctx: MinigameContext): Promise<void> {
    ...
    this.game3d = new BabylonVolleyballGame(canvas, ctx);
  }
...
class VolleyballPauseMenu {
  ...
  constructor(
    ...
    title.textContent = '🏐 PALLAVOLO DEI DISAGIATI — PAUSA';
